/*
 *  This file was copied locally to ~/Arduino/Ovidius folder from source:
 *  
 ...
 */
PseudoSPIcommMetamorphicManipulator SLAVE_SPI(Rx, pseudoID,  MOSI_NANO, MISO_NANO, SCK_NANO, redLED_Pin, greenLED_Pin, blueLED_Pin, ssPins);
...
// SLAVE INTERNAL ISR CONFIGURATION
  ...
  SPI.attachInterrupt();
  //digitalWrite(greenLED_Pin,LOW); digitalWrite(blueLED_Pin,LOW);
  
// READ SETTINGS FROM EEPROM
  //SLAVE_SPI..setupEEPROMslave( pseudoID, PI/2, -PI/2, 0.20944);   // LAST EXECUTION Wed 18.3.2020

  //SLAVE_SPI..readEEPROMsettingsSlave( pseudoID, &motor_new_state ,  &currentAbsPosPseudo_ci,  &currentDirStatusPseudo, &currentAbsPosPseudo);

  //return_function_state = SLAVE_SPI..setHomePositionSlave( &motor_new_state, &currentAbsPosPseudo, &currentAbsPosPseudo_ci, &currentDirStatusPseudo, &homingHallActivated, &limitHallActivated);
 
  //return_function_state = SLAVE_SPI..saveEEPROMsettingsSlave( &motor_new_state, &currentAbsPosPseudo_ci , &currentDirStatusPseudo);
...
void loop (void)
{
  ...
  if ( CONNECT2MASTER )
  {
        ...
        slaveID  = SLAVE_SPI.connectPseudoSlave();
        ...
        if (slaveID == pseudoID)            // ID values must agree in order connection to be achieved
        {
          ...
          SLAVE_SPI.setPseudoLed(pseudo_pinged_indicator);
          Serial.println("[   INFO   ]  CONNECTED");
        }
        else
        {
          connected2master = false;          // Reading ID from EEPROM completed...
          SLAVE_SPI.setPseudoLed(pseudo_critical_error_led);
  ...
  if ( GIVE_CS )
  {
        return_function_state = SLAVE_SPI.readCurrentStateSlave(&motor_new_state);
  ...
  if ( GIVE_CP )
  {
        return_function_state = SLAVE_SPI.readCurrentAnatomySlave(&motor_current_ci, &CURRENT_Ci_IDENTITY);
        if (return_function_state)
        {
  ...
  if ( SET_GOAL_POS )
  {
        ...
        return_function_state = SLAVE_SPI.setGoalPositionSlave2( &PSEUDO_GOAL_POSITION, &currentAbsPosPseudo_ci, &RELATIVE_STEPS_TO_MOVE, &currentDirStatusPseudo, &motor_new_state );
        if (return_function_state)
        {
  ...
  if ( MOVE_MOTOR )
  {
        ...
        return_function_state = SLAVE_SPI.movePseudoSlave(&motor_new_state , &RELATIVE_STEPS_TO_MOVE, &limitHallActivated, &operation_executed);
        if (return_function_state)
        {
  ...
  if ( LOCK_MOTOR )
  {
        ...
        return_function_state = SLAVE_SPI.lockPseudoSlave(&motor_new_state );
  ...
  if ( UNLOCK_MOTOR )
  {
        ...
        return_function_state = SLAVE_SPI.unlockPseudoSlave(&motor_new_state );
  ...
  {
        // Calls function lockPseudoSlave
        return_function_state = SLAVE_SPI.setPreHomePositionStateSlave( &motor_new_state);
  ...
  {
        // Calls function lockPseudoSlave
        return_function_state = SLAVE_SPI.go2HomePositionSlave( &motor_new_state, &currentAbsPosPseudo, &currentAbsPosPseudo_ci, &currentDirStatusPseudo, &homingHallActivated, &limitHallActivated, &operation_executed);
...
/*
  ...
  if ( INDICATE_META_REPEATS )      // only indicates meta repeats, global not need to be saved - no fn executed
  {
        ...
        //SLAVE_SPI..txrxLEDSblink(3, 1500);
        //Serial.print("State here must be locked(100) : "); Serial.println(motor_new_state);
        return_function_state = SLAVE_SPI..saveCurrentStateSlave(&motor_new_state);
        if (return_function_state)
        {
...
*/
  if ( SAVE_EEPROM )     
  {
        ...
        Serial.print("Last operation executed was: 150(home)/151(meta):"); Serial.println(operation_executed);
        
        return_function_state = SLAVE_SPI.saveEEPROMsettingsSlave( &motor_new_state, &currentAbsPosPseudo_ci , &currentDirStatusPseudo, &operation_executed);
        if (return_function_state)
        {
          Serial.print("[   PSEUDO:"); Serial.print(pseudoID); Serial.print("   ]   [   CURRENT STATUS:"); Serial.print(STATE_SAVED_EEPROM_STRING); Serial.println("   ]   SUCCESS");          
          saved_to_eeprom = true;
        }
        else
        {
          Serial.print("[   PSEUDO:"); Serial.print(pseudoID); Serial.print("   ]   [   CURRENT STATUS:"); Serial.print(STATE_SAVED_EEPROM_STRING); Serial.println("   ]   FAILED");          
          saved_to_eeprom = false;
        }
        time_now_micros = micros();
        while(micros() < time_now_micros + 500){}  }
