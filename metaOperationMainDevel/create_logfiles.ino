void create_logfiles()
{
  ...
  char * PTRS2SENSOR_DIRS[3]; // must be expanded to 5!
  ...
  for (size_t i = 0; i < sizeof(LOG_FILES); i++)
  {
    //LOG_FILE_TEMP = LOG_FILES[i];
    
    RobotDataLog.createFile(LOGFILES+i, PTRS2SENSOR_DIRS[i], LOG_FILE_PATH[i], &data_error);
    if (data_error == NO_ERROR)
    {
      DEBUG_SERIAL.print(F("[ SETUP ] CREATE LOG FILE SUCCESS: "));
      DEBUG_SERIAL.println(LOG_FILE_PATH[i]);
    }
    else
    {
      DEBUG_SERIAL.println(F("[ SETUP ] CREATE LOG FILE FAILED"));
    }
  }
