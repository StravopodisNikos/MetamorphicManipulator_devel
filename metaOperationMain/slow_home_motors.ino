  void slow_home_motors()
  {
    ...
    DEBUG_SERIAL.println("[    INFO    ] SLOW HOMING STEPPER MOTOR [  SUCCESS ]");
    DEBUG_SERIAL.print("[  ERROR CODE  ]");DEBUG_SERIAL.println(error_code_received);
    }
    else
    {
      DEBUG_SERIAL.println("[    ERROR   ] SLOW HOMING STEPPER MOTOR [  FAILED ]");
      DEBUG_SERIAL.print("[  ERROR CODE  ]");DEBUG_SERIAL.println(error_code_received);
    }
    ...
    delay(5000); // wait for Dynamixels to HOME
